...
debug_draw_colliders :: proc(dc: ^Debug_Context, world: ^core.World) {
	// Draw all colliders (unified query)
	for arch in ecs.query(world.ecs, ecs.has(physics.Collider), ecs.has(core.Transform)) {
		colliders := ecs.get_table(world.ecs, arch, physics.Collider)
		transforms := ecs.get_table(world.ecs, arch, core.Transform)

		for i in 0..<len(colliders) {
			col := &colliders[i]
			if !col._initialized { continue }
			...
			case physics.Circle:
				...
				end_x := pos.x + math.cos(rot) * shape.radius
				end_y := pos.y + math.sin(rot) * shape.radius
				k2.draw_line(
					k2.Vec2{pos.x, pos.y},
					k2.Vec2{end_x, end_y},
					2,
...
debug_draw_velocities :: proc(dc: ^Debug_Context, world: ^core.World) {
	// Only Rigidbody entities have meaningful velocity
	for arch in ecs.query(world.ecs, ecs.has(physics.Rigidbody)) {
		bodies := ecs.get_table(world.ecs, arch, physics.Rigidbody)

		for i in 0..<len(bodies) {
			rb := &bodies[i]
			if !rb._initialized { continue }
			draw_velocity_arrow(rb._body_id, dc.velocity_color)
		}
	}
}
