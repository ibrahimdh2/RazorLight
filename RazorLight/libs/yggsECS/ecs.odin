...
World :: struct {
	...
	remove_callbacks:     map[ComponentID]proc(rawptr),
}
...
create_world :: proc() -> ^World {
	...
	world.remove_callbacks = make(map[ComponentID]proc(rawptr))
	return world
}
...
remove_entity :: proc(world: ^World, entity: EntityID) {
	...
	// Invoke remove callbacks for all components before removal
	for component_id in archetype.component_ids {
		if callback, has_cb := world.remove_callbacks[component_id]; has_cb {
			if table, exists := archetype.tables[component_id]; exists {
				type_info := archetype.component_types[component_id]
				size := size_of_type(type_info)
				if len(table) > 0 && row * size < len(table) {
					component_ptr := rawptr(&table[row * size])
...
get_component_same :: proc(world: ^World, entity: EntityID, $Component: typeid) -> ^Component {
    ...
    return &components[row]  // FIXED: Changed ^ to &
}
...
execute :: proc(q: ^QueryBuilder) -> []^Archetype {
	...
	for term in q.terms {
		...
		// Unregistered not-terms are trivially satisfied (no entity can have an unregistered component)
		if !ok && !is_not {
			return []^Archetype{}
		}
	}
...
// ============================================================================
// Convenience API: get (alias for get_component)
// ============================================================================

get :: get_component

// ============================================================================
// Lifecycle Hooks: on_remove callbacks
// ============================================================================

// Register a callback to be invoked when a component is removed from an entity.
// The callback receives a rawptr to the component data - cast it to your component type.
// Example:
//   ecs.on_remove(world, PhysicsBody, proc(ptr: rawptr) {
//       body := cast(^PhysicsBody)ptr
//       b2.DestroyBody(body.body_id)
//   })
on_remove :: proc(world: ^World, $T: typeid, callback: proc(rawptr)) {
	cid, ok := get_component_id(world, T)
	if !ok {
		cid = register_component(world, T)
	}
	world.remove_callbacks[cid] = callback
}
...
iter1 :: proc(world: ^World, $A: typeid) -> QueryIter1(A) {
	archetypes := query(world, has(A))
	return QueryIter1(A){
		world      = world,
...
iter1_next :: proc(it: ^QueryIter1($A)) -> (entity: EntityID, a: ^A, ok: bool) {
	for it.arch_idx < len(it.archetypes) {
		arch := it.archetypes[it.arch_idx]

		if it.row_idx == 0 {
			it.table_a = get_table(it.world, arch, A)
			it.entities = arch.entities[:]
		}

		if it.row_idx < len(it.entities) {
			idx := it.row_idx
			it.row_idx += 1
			return it.entities[idx], &it.table_a[idx], true
		}
...
// 2-component iterator
QueryIter2 :: struct($A, $B: typeid) {
	world:      ^World,
	archetypes: []^Archetype,
	arch_idx:   int,
	row_idx:    int,
...
iter2 :: proc(world: ^World, $A, $B: typeid) -> QueryIter2(A, B) {
	archetypes := query(world, has(A), has(B))
	return QueryIter2(A, B){
		world      = world,
		archetypes = archetypes,
	}
}

iter2_next :: proc(it: ^QueryIter2($A, $B)) -> (entity: EntityID, a: ^A, b: ^B, ok: bool) {
	for it.arch_idx < len(it.archetypes) {
		arch := it.archetypes[it.arch_idx]

		if it.row_idx == 0 {
			it.table_a = get_table(it.world, arch, A)
			it.table_b = get_table(it.world, arch, B)
			it.entities = arch.entities[:]
		}
		...
		if it.row_idx < len(it.entities) {
			idx := it.row_idx
			it.row_idx += 1
			return it.entities[idx], &it.table_a[idx], &it.table_b[idx], true
		}
...
// 3-component iterator
QueryIter3 :: struct($A, $B, $C: typeid) {
	world:      ^World,
	archetypes: []^Archetype,
	arch_idx:   int,
	row_idx:    int,
...
iter3 :: proc(world: ^World, $A, $B, $C: typeid) -> QueryIter3(A, B, C) {
	archetypes := query(world, has(A), has(B), has(C))
	return QueryIter3(A, B, C){
		world      = world,
		archetypes = archetypes,
	}
}

iter3_next :: proc(it: ^QueryIter3($A, $B, $C)) -> (entity: EntityID, a: ^A, b: ^B, c: ^C, ok: bool) {
	for it.arch_idx < len(it.archetypes) {
		arch := it.archetypes[it.arch_idx]
		...
		if it.row_idx == 0 {
			it.table_a = get_table(it.world, arch, A)
			it.table_b = get_table(it.world, arch, B)
			it.table_c = get_table(it.world, arch, C)
			it.entities = arch.entities[:]
		}
		...
		if it.row_idx < len(it.entities) {
			idx := it.row_idx
			it.row_idx += 1
			return it.entities[idx], &it.table_a[idx], &it.table_b[idx], &it.table_c[idx], true
		}
...
// 4-component iterator
QueryIter4 :: struct($A, $B, $C, $D: typeid) {
	world:      ^World,
	archetypes: []^Archetype,
	arch_idx:   int,
	row_idx:    int,
...
iter4 :: proc(world: ^World, $A, $B, $C, $D: typeid) -> QueryIter4(A, B, C, D) {
	archetypes := query(world, has(A), has(B), has(C), has(D))
	return QueryIter4(A, B, C, D){
		world      = world,
		archetypes = archetypes,
	}
}

iter4_next :: proc(it: ^QueryIter4($A, $B, $C, $D)) -> (entity: EntityID, a: ^A, b: ^B, c: ^C, d: ^D, ok: bool) {
	for it.arch_idx < len(it.archetypes) {
		arch := it.archetypes[it.arch_idx]
		...
		if it.row_idx == 0 {
			...
			it.table_b = get_table(it.world, arch, B)
			it.table_c = get_table(it.world, arch, C)
			it.table_d = get_table(it.world, arch, D)
			it.entities = arch.entities[:]
		}
		...
		if it.row_idx < len(it.entities) {
			idx := it.row_idx
			it.row_idx += 1
			return it.entities[idx], &it.table_a[idx], &it.table_b[idx], &it.table_c[idx], &it.table_d[idx], true
		}
...
spawn :: proc(world: ^World) -> EntityBuilder {
...
with :: proc(builder: EntityBuilder, component: $T) -> EntityBuilder {
	add_component(builder.world, builder.entity, component)
	return builder
}

build :: proc(builder: EntityBuilder) -> EntityID {
